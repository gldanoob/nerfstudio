...
class Pipeline():
    ...
    def run(self, cleaning: str, clustering: str, smoothing: str, post_processing: str, stop_at: str = 'all', **kwargs):
        ...
        if stop_at != 'smoothing' and CACHING and (self.out_path.parent / f'{smoothing}.ply').exists():
            ...
            if stop_at == 'clustering':
                self.plot_clusterings(f'results/{self.out_path.parent.name}_{clustering}.png', m_cleaned.vertex_matrix(), clustering_all, N_CLUSTERS)
                return
            ...
            for i in range(N_CLUSTERS):
                ...
                for j, subcluster_mesh in enumerate(cluster_meshes):
                    ...
                    if EVAL_LEAF_HEURISTIC and j < 10 and subcluster_mesh.faces.shape[0] > N_LEAF_TRIANGLES:
                        dist = self.mesh_to_leaf_gt(subcluster_mesh, self.gt_dir / 'leaf.ply')
                        if not math.isnan(dist):
                            predicted.append(self.is_leaf_heuristic(subcluster_mesh))
            ...
            if EVAL_LEAF_HEURISTIC:
                predicted = np.array(predicted)
                distances = np.array(distances) * 1e4

                np.savez(f'results/{self.out_path.parent.name}_heuristic.npz', predicted=predicted, distances=distances)
    ...
    def remove_with_dbscan(self, mesh: trimesh.Trimesh, n_samples, eps, min_samples) -> trimesh.Trimesh:
        ...
        lprint('Number of outliers:', len(outlier_indices))
        outlier_subfaces = np.any(np.isin(mesh.faces, outlier_indices), axis=1)

        lprint(outlier_subfaces.sum(), 'faces removed')
    ...
    def plot_clusterings(self, fname, X, clustering, n_clusters):
    ...
    def mesh_to_leaf_gt(self, mesh: trimesh.Trimesh, gt):
        mesh.export('tmp/tmp.ply')

        res = eval_mesh.hausdorff_distance(
            source=pathlib.Path('tmp/tmp.ply'),
            target=gt
        )
