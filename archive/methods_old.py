...
REMOVE_SMALL_CLUSTERS = 50
N_CLUSTERS = 150
LEAF_VARIANCE_THRESHOLD = 0.65


class Method:
    __name__ = "Method"

    def __init__(self, in_path: pathlib.Path, out_path: pathlib.Path, tmp_path=None):
        self.in_path = in_path.as_posix()
        self.out_path = out_path.as_posix()
...
class CloseHoles(Method):
    ...
    def run(self):
        ...
        ms.meshing_close_holes(maxholesize=100, newfaceselected=False, refinehole=True)
        ms.save_current_mesh(self.out_path)
...
def pymesh_to_trimesh(mesh: pymeshlab.Mesh):
...
class ClusteringSKLearn(Method):
    def run(self):
        ...
        # Decimate the mesh (assuming vertex number linearly scales with face number)
        # target_faces = int(8000 / m.vertex_number() * m.face_number())
        # ms.meshing_decimation_quadric_edge_collapse(targetfacenum=target_faces, preserveboundary=True)
        ...
        affinity = pairwise_kernels(X, metric='rbf', gamma=15)
        ...
        for i in range(N_CLUSTERS):
            ...
            subfaces = np.any(np.isin(faces, cluster_indices), axis=1)
            subfaces_list.append(subfaces)

        subfaces_list.sort(key=lambda x: x.sum(), reverse=True)
        submeshed = trimesh_mesh.submesh(subfaces_list, append=False)

        # process each submesh
        ms_new = pymeshlab.MeshSet(verbose=True)
        for i, submesh in enumerate(submeshed):
            ms_new.add_mesh(trimesh_to_pymesh(submesh))
            if self.is_leaf_heuristic(submesh):
                submesh.export(f'tmp/tmp_{i}.ply')
                print('Smoothing leaf cluster #', i)
                self.smooth_leaf(ms_new)
    ...
    def is_leaf_heuristic(self, mesh: trimesh.Trimesh) -> bool:
        ...
        cov = np.cov(face_normals.T)
        assert cov.shape == (3, 3)
        var = np.trace(cov)  # sum of variances
        print('Variance:', var)
        return var < LEAF_VARIANCE_THRESHOLD

    def smooth_leaf(self, ms: pymeshlab.MeshSet):
        face_normals = ms.current_mesh().face_normal_matrix().copy()
        # flip each normal if z component is negative
        face_normals[face_normals[:, 2] < 0] *= -1
        ...
        # depth smooth
        ms.apply_coord_depth_smoothing(stepsmoothnum=3, viewpoint=viewpoint)
